...
// NOTE: Tagalog model is not supported on the new HF router (router.huggingface.co).
// Tagalog profanity is handled by the local profanity-list.json + emergency blocklist instead.
const TAGALOG_MODEL = 'ggpt1006/tl-hatespeech-detection' // Disabled — not supported by hf-inference provider
...
const TAGALOG_THRESHOLD  = 0.60  // 60% — Tagalog model threshold
 ...
 * Step 3: Tagalog model (ggpt1006/tl-hatespeech-detection) — LABEL_0/LABEL_1
...
// ─────────────────────────────────────────────────────────────
// PRIVATE: Tagalog model
// ggpt1006/tl-hatespeech-detection returns:
// [[{ label: "LABEL_0", score: 0.1 }, { label: "LABEL_1", score: 0.9 }]]
// LABEL_0 = clean, LABEL_1 = hate speech/profanity
// ─────────────────────────────────────────────────────────────
async function _moderateTextTagalog(text, apiKey) {
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      const response = await fetch(`${HF_API_BASE}/${TAGALOG_MODEL}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${apiKey}`,
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errText = await response.text()
        console.error(`AI Text (TL) HTTP ${response.status}:`, errText)
        if (response.status === 503 && attempt < MAX_RETRIES) {
          await sleep(RETRY_DELAY_MS)
          continue
        }
        return { flagged: false, score: 0, raw: errText }
      }
      ...
      if (result.error && typeof result.error === 'string' && result.error.includes('loading')) {
        ...
        return { flagged: false, score: 0, raw: result }
      }
      ...
      // LABEL_1 = hate speech
      const hateResult = scores.find(r => r.label === 'LABEL_1')
      const hateScore = hateResult?.score || 0
      const flagged = hateScore >= TAGALOG_THRESHOLD

      return { flagged, score: hateScore, raw: result }
    } catch (err) {
      console.error(`AI Text (TL) error (attempt ${attempt + 1}):`, err.message)
      if (attempt < MAX_RETRIES) { await sleep(RETRY_DELAY_MS); continue }
      return { flagged: false, score: 0, raw: err.message }
    }
  }
  return { flagged: false, score: 0, raw: 'max retries exceeded' }
}
