 ...
 */
export async function sendBanNotification(email, fullName, reason) {
    const transporter = getTransporter();
    if (!transporter) return { success: false, reason: 'SMTP not configured' };
    ...
    const reasonBlock = reason
        ? `<div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:16px;padding:20px;margin:20px 0;">
            <p style="margin:0 0 6px;color:#ef4444;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:1px;">Reason for Suspension</p>
            <p style="margin:0;color:rgba(255,255,255,0.7);font-size:14px;">${reason}</p>
           </div>`
        : '';
    ...
    const html = wrapHtml(
        ...
        <p style="color:rgba(255,255,255,0.7);font-size:15px;line-height:1.7;margin:0 0 20px;">
            Your FoundIt account has been <strong style="color:#ef4444;">suspended</strong> by an administrator. While suspended, you will not be able to access the platform or any of its features.
        </p>
        ${reasonBlock}
        <p style="color:rgba(255,255,255,0.4);font-size:13px;margin:0;">
            If you believe this action was taken in error, please contact the administration office for assistance.
        </p>`
    );
    ...
    try {
        await transporter.sendMail({
            from: `"FoundIt LSPU" <${process.env.SMTP_EMAIL}>`,
            to: email,
            subject: '🚫 Your FoundIt Account Has Been Suspended',
            html,
        });
        return { success: true };
    } catch (err) {
        console.error('Email send error (ban):', err);
        return { success: false, reason: err.message };
    }
}

/**
 * Send unban notification email to user.
 */
export async function sendUnbanNotification(email, fullName, reason) {
    const transporter = getTransporter();
    if (!transporter) return { success: false, reason: 'SMTP not configured' };
    ...
    const reasonBlock = reason
        ? `<div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:16px;padding:20px;margin:20px 0;">
            <p style="margin:0 0 6px;color:#22c55e;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:1px;">Reason</p>
            <p style="margin:0;color:rgba(255,255,255,0.7);font-size:14px;">${reason}</p>
           </div>`
        : '';
    ...
    const html = wrapHtml(
        ...
        <p style="color:rgba(255,255,255,0.7);font-size:15px;line-height:1.7;margin:0 0 20px;">
            Great news! Your FoundIt account has been <strong style="color:#22c55e;">reinstated</strong>. Your suspension has been lifted and you can now access all platform features again.
        </p>
        ${reasonBlock}
        <div style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:16px;padding:20px;margin:20px 0;">
            <p style="margin:0;color:#22c55e;font-size:13px;font-weight:700;">🎉 Your account is now fully active again</p>
        </div>
        <p style="color:rgba(255,255,255,0.4);font-size:13px;margin:0;">
            Please ensure you follow the community guidelines to avoid future suspensions. Log in to start using FoundIt again.
        </p>`
    );
    ...
    try {
        await transporter.sendMail({
            from: `"FoundIt LSPU" <${process.env.SMTP_EMAIL}>`,
            to: email,
            subject: '✅ Your FoundIt Account Has Been Reinstated',
            html,
        });
        return { success: true };
    } catch (err) {
        console.error('Email send error (unban):', err);
        return { success: false, reason: err.message };
    }
}
